...
// Dont duplicate me!
if ( ! class_exists( 'Ktc_Nav_Walker' ) ) {

	/**
	 * The main navwalker.
	 */
	class Ktc_Nav_Walker extends Walker_Nav_Menu {
		...
		private function set_megamenu_max_width() {
			...
			$site_width         = (int) cs_get_option( 'site-width' );
			$megamenu_max_width = $site_width;
			$megmanu_width      = 0;
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			if ( 0 === $depth && 'enabled' === $this->menu_megamenu_status ) {
				...
				$output .= '{first_level}';
				$output .= '<div class="rab-megamenu-holder" {megamenu_final_width}><ul class="rab-megamenu {megamenu_border}">';
			} elseif ( 2 <= $depth && 'enabled' === $this->menu_megamenu_status ) {
				$output .= '<ul class="sub-menu deep-level">';
			} elseif( 'header-3' === $args->theme_location && '' === $this->menu_megamenu_status ) {
				// We have to modify output a little to achieve our style.
				$output .= '<ul class="dl-submenu">';
					$output .= '<li class="dl-back">';
						$output .= '<a href="#">'.esc_html__( 'Back', 'evostore' ).'</a>';
					$output .= '</li>';
			} else {
		...
		public function end_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if ( 0 === $depth && 'enabled' === $this->menu_megamenu_status ) {
				...
				if ( 'fullwidth' === $this->menu_megamenu_width ) {
					$col_span = ' col-span-12 rab-megamenu-fullwidth';
				...
				$output = str_replace( '{first_level}', '<div class="mega-menu dropdown-menu rab-dropdown-menu rab-megamenu-wrapper {rab_columns} columns-' . $this->total_num_of_columns . $col_span . '" data-maxwidth="' . $this->menu_megamenu_maxwidth . '"><div class="row mega-wrap">', $output );

				$output = str_replace( '{megamenu_final_width}', 'style="width:' . $wrapper_width . 'px;" data-width="' . $wrapper_width . '"', $output );

				if ( $this->total_num_of_columns > $this->max_num_of_columns ) {
					$output = str_replace( '{megamenu_border}', 'rab-megamenu-border', $output );
				} else {
				...
				foreach ( $this->submenu_matrix as $row => $columns ) {
					...
					if ( ( $row - 1 ) * $this->max_num_of_columns + $columns < $this->total_num_of_columns ) {
						$output = str_replace( '{row_number_' . $row . '}', 'rab-megamenu-row-columns-' . $columns . ' rab-megamenu-border', $output );
					} else {
						$output = str_replace( '{row_number_' . $row . '}', 'rab-megamenu-row-columns-' . $columns, $output );
					}
					$output = str_replace( '{current_row_' . $row . '}', 'rab-megamenu-columns-' . $columns . ' col-lg-' . $layout_columns . ' col-md-' . $layout_columns . ' col-sm-' . $layout_columns, $output );

					$output = str_replace( '{rab_columns}', 'rab-columns-' . $columns . ' columns-per-row-' . $columns, $output );
				}
			} else {
				$output .= '</ul>';
			}
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( $is_mega_menu ) {
				if ( 0 === $depth ) {
					$this->menu_megamenu_status = get_post_meta( $item->ID, '_menu_item_rab_megamenu_status', true );
					$this->menu_megamenu_width  = get_post_meta( $item->ID, '_menu_item_rab_megamenu_width', true );
					$allowed_columns = get_post_meta( $item->ID, '_menu_item_rab_megamenu_columns', true );
				...
				$this->menu_style               = get_post_meta( $item->ID, '_menu_item_rab_menu_style', true );
				$this->menu_megamenu_title      = get_post_meta( $item->ID, '_menu_item_rab_megamenu_title', true );
				$this->menu_megamenu_widgetarea = get_post_meta( $item->ID, '_menu_item_rab_megamenu_widgetarea', true );
				$this->menu_megamenu_icon       = get_post_meta( $item->ID, '_menu_item_rab_megamenu_icon', true );
				$this->menu_megamenu_thumbnail  = get_post_meta( $item->ID, '_menu_item_rab_megamenu_thumbnail', true );
			...
			if ( 1 === $depth && 'enabled' === $this->menu_megamenu_status ) {
				...
				if ( get_post_meta( $item->ID, '_menu_item_rab_megamenu_columnwidth', true ) ) {
					$this->menu_megamenu_columnwidth = get_post_meta( $item->ID, '_menu_item_rab_megamenu_columnwidth', true );
				} else {
				...
				if ( $this->num_of_columns > $this->max_num_of_columns ) {
					...
					$output .= '</ul><ul class="rab-megamenu rab-megamenu-row-' . $this->num_of_rows . ' {row_number_' . $this->num_of_rows . '}" {row_width_' . $this->num_of_rows . '}>';
				} else {
					$this->menu_megamenu_rowwidth_matrix[ $this->num_of_rows ] += floatval( $this->menu_megamenu_columnwidth ) / 100;
				}
				...
				if ( ! ( ( empty( $item->url ) || '#' === $item->url || 'http://' === $item->url )  && 'disabled' === $this->menu_megamenu_title ) ) {
					...
					if ( ! empty( $item->url ) && '#' !== $item->url && 'http://' !== $item->url ) {
						...
						if ( 'disabled' === $this->menu_megamenu_title ) {
							$link_class = ' class="rab-megamenu-title-disabled"';
						}

						$link         = '<a href="' . esc_url( $item->url ) . '"' . $target . $link_class . '>';
						$link_closing = '</a>';
					}
					...
					if ( ! empty( $this->menu_megamenu_thumbnail ) ) {
						$title_enhance = '<span class="rab-megamenu-icon"><img src="' . esc_url( $this->menu_megamenu_thumbnail ) . '"></span>';
					} elseif ( ! empty( $this->menu_megamenu_icon ) ) {
						$title_enhance = '<span class="rab-megamenu-icon"><i class="fa glyphicon ' . $this->menu_megamenu_icon . '"></i></span>';
					} elseif ( 'disabled' === $this->menu_megamenu_title ) {
						$title_enhance = '<span class="rab-megamenu-bullet"></span>';
					}

					$heading = $link . $title_enhance . $title . $link_closing;

					if ( 'disabled' !== $this->menu_megamenu_title ) {
						$item_output .= "<div class='rab-megamenu-title'>" . $heading . '</div>';
					} else {
						$item_output .= $heading;
					}
				}
				...
				if ( $this->menu_megamenu_widgetarea && is_active_sidebar( $this->menu_megamenu_widgetarea ) ) {
					...
					$item_output .= '<div class="rab-megamenu-widgets-container second-level-widget">' . ob_get_clean() . '</div>';
				}
				...
				if ( is_active_sidebar( $this->menu_megamenu_widgetarea ) ) {
					ob_start();
					dynamic_sidebar( $this->menu_megamenu_widgetarea );
					$item_output .= '<div class="rab-megamenu-widgets-container third-level-widget">' . ob_get_clean() . '</div>';
				}
			} else {
				...
				// heading style is selected so replace a with h5.
				if( $this->menu_style === 'rab-heading' ) {
					$item_output .= $args->before . '<h5 class="menu-title">';
				} else {
					$item_output .= $args->before . '<a ' . $attributes . '>';
				}

				// Check if we need to set an image.
				$icon_wrapper_class = 'rab-megamenu-icon';
				...
				if ( ! empty( $this->menu_megamenu_thumbnail ) && 'enabled' === $this->menu_megamenu_status ) {
					$icon = '<span class="' . $icon_wrapper_class . ' rab-megamenu-image"><img src="' . $this->menu_megamenu_thumbnail . '"></span>';
				} elseif ( ! empty( $this->menu_megamenu_icon ) ) {
					...
					$icon = '<span class="rab-megamenu-bullet"></span>';
				}
				...
				if ( 0 === $depth ) {
					$classes = 'menu-text';
					if ( $this->menu_style && 'rab-heading' !== $this->menu_style ) {
						$classes .= ' rab-button button-default ' . str_replace( 'rab-', '', $this->menu_style );
					} else if( 'rab-heading' === $this->menu_style ) {
						$classes .= ' rab-heading-style';
					}
				...
				if ( false !== strpos( $icon, 'button-icon-divider-left' ) ) {
					$title = '<span class="rab-button-text-left">' . $title . '</span>';
				} elseif ( false !== strpos( $icon, 'button-icon-divider-right' ) ) {
					$title = '<span class="rab-button-text-right">' . $title . '</span>';
				}
			...
			if ( 'enabled' !== $this->menu_megamenu_status && ( ( 0 == strcasecmp( $item->attr_title, 'divider' ) ) || ( 0 == strcasecmp( $item->title, 'divider' ) ) ) ) {
				...
				if ( 0 === $depth && $args->has_children ) {
					$class_names .= ( 'enabled' === $this->menu_megamenu_status ) ? ' rab-megamenu-menu mega-link' : ' rab-dropdown-menu dropdown';
				}

				if ( 0 === $depth && $this->menu_style ) {
					$class_names .= ' rab-menu-item-button';
				}

				if ( 1 === $depth ) {
					if ( 'enabled' === $this->menu_megamenu_status ) {
						$class_names .= ' rab-megamenu-submenu';

						if ( 'disabled' === $this->menu_megamenu_title ) {
							$class_names .= ' rab-megamenu-submenu-notitle';
						}
						...
						$class_names .= ' rab-dropdown-submenu';
					}
				}
