(function( $ ){
    ...
    Raw_Framework_Sidebar.prototype = {
        // grab our form and display it in the widgets screen
        ...
        append_remove_button: function() {
            ...
            var string = '<span class="rab-remove-sidebar"><i class="dashicons dashicons-trash"></i></span>';
            area.find('.sidebar-rab').append(string);
        },

        // bind events to the remove icon so that custom sidebars can be removed
        bind_events: function() {
            this.wrapper.on('click', '.rab-remove-sidebar', $.proxy( this.delete_sidebar, this));
        },

        //delete the sidebar area with all widgets
        delete_sidebar: function(e) {
            // ask for the confirmation
            var process = confirm(rabWidgets.sidebarConfirm);
