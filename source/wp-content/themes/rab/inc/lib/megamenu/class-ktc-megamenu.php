...
if ( ! class_exists( 'Ktc_Megamenu' ) ) {
	 ...
	 * @since 3.4
	 */
	class Ktc_Megamenu extends Ktc_Megamenu_Core {
		...
		/**
		 * Function to replace normal edit nav walker for rab core mega menus.
		 *
		 * @return string Class name of new navwalker
		 */
		public function add_custom_fields() {
			return 'Ktc_Nav_Walker_Megamenu';
		}
		...
		public function save_custom_menu_style_fields( $menu_id, $menu_item_db_id, $args ) {
			$field_names = array( 'menu-item-rab-megamenu-icon' );
			if ( ! $args['menu-item-parent-id'] ) {
				$field_names = array( 'menu-item-rab-menu-style', 'menu-item-rab-megamenu-icon' );
			}
		...
		public function add_menu_style_data_to_menu( $menu_item ) {
			if ( ! $menu_item->menu_item_parent ) {
				$menu_item->rab_menu_style = get_post_meta( $menu_item->ID, '_menu_item_rab_menu_style', true );
			}

			$menu_item->rab_megamenu_icon = get_post_meta( $menu_item->ID, '_menu_item_rab_megamenu_icon', true );
		...
		public function save_custom_megamenu_fields( $menu_id, $menu_item_db_id, $args ) {
			...
			foreach ( $field_name_suffix as $key ) {
				if ( ! isset( $_REQUEST[ 'menu-item-rab-megamenu-' . $key ][ $menu_item_db_id ] ) ) {
					$_REQUEST[ 'menu-item-rab-megamenu-' . $key ][ $menu_item_db_id ] = '';
				}
				$value = $_REQUEST[ 'menu-item-rab-megamenu-' . $key ][ $menu_item_db_id ];
				update_post_meta( $menu_item_db_id, '_menu_item_rab_megamenu_' . $key, $value );
			}
		}
		...
		public function add_megamenu_data_to_menu( $menu_item ) {
			...
			if ( ! $menu_item->menu_item_parent ) {
				...
				$menu_item->rab_megamenu_status  = get_post_meta( $menu_item->ID, '_menu_item_rab_megamenu_status', true );
				$menu_item->rab_megamenu_width   = get_post_meta( $menu_item->ID, '_menu_item_rab_megamenu_width', true );
				$menu_item->rab_megamenu_columns = get_post_meta( $menu_item->ID, '_menu_item_rab_megamenu_columns', true );

			} else {

				$menu_item->rab_megamenu_title      = get_post_meta( $menu_item->ID, '_menu_item_rab_megamenu_title', true );
				$menu_item->rab_megamenu_widgetarea = get_post_meta( $menu_item->ID, '_menu_item_rab_megamenu_widgetarea', true );

			}

			$menu_item->rab_megamenu_columnwidth = get_post_meta( $menu_item->ID, '_menu_item_rab_megamenu_columnwidth', true );
			$menu_item->rab_megamenu_icon        = get_post_meta( $menu_item->ID, '_menu_item_rab_megamenu_icon', true );
			$menu_item->rab_megamenu_thumbnail   = get_post_meta( $menu_item->ID, '_menu_item_rab_megamenu_thumbnail', true );
