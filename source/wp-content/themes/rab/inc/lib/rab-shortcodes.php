...
$GLOBALS["available_fonts"]= array(
    $bodyFont = key((Array)json_decode(cs_get_option('font-body'))),
    $navFont  = key((Array)json_decode(cs_get_option('font-navigation'))),
    $headFont = key((Array)json_decode(cs_get_option('font-headings'))),
    $ShortcodeFont = key((Array)json_decode(cs_get_option('font-shortcode')))
);
/*-----------------------------------------------------------------------------------*/
/*  Shortcode forms ajax handler
/*-----------------------------------------------------------------------------------*/

function rab_sc_popup()
{   
    include('forms.php');
    die();
}

add_action('wp_ajax_ep_sc_popup', 'rab_sc_popup');
...
function rab_sc_separator( $atts, $content = null ) {
...
function rab_sc_title( $atts, $content = null ) {
    extract(shortcode_atts(array(
        'title'   => 'Title',
    ...
    $id = rab_sc_id('vc_text_separator');
...
function rab_sc_team_member( $atts, $content = null ) {
    ...
    $id     = rab_sc_id('team_member');
    ob_start();
    ?>
        <div id="<?php echo $id; ?>" class="leader-box text-center">
            ...
            <span><?php echo esc_attr( $job_title ); ?></span>

            <ul class="social-icons">
            <?php 
            if( $facebook_url ):
                ?>
                <li> <a target="<?php echo $team_icon_target1; ?>" href="<?php echo esc_url( $facebook_url ); ?>"><i class="fa fa-facebook"></i></a> </li>
                <?php
            endif;
            ?>
            ...
            if( $twitter_url ):
                ?>
                <li> <a target="<?php echo $team_icon_target2; ?>" href="<?php echo esc_url( $twitter_url ); ?>"><i class="fa fa-twitter"></i></a> </li>
                <?php
            endif;
            ?>
            ...
            if( $linkedin_url ):
                ?>
                <li> <a target="<?php echo $team_icon_target3; ?>" href="<?php echo esc_url( $linkedin_url ); ?>"><i class="fa fa-linkedin"></i></a> </li>
                <?php
            endif;
            ?>
            ...
            if( $instagram_url ):
                ?>
                <li> <a target="<?php echo $team_icon_target4; ?>" href="<?php echo esc_url( $instagram_url ); ?>"><i class="fa fa-instagram"></i></a> </li>
                <?php
            endif;
            ?>

            <?php 
            if( $custom_icon !== '' && ( $custom_url !== '' || $custom_url !== '#' ) ):
                ?>
                <li> <a target="<?php echo $team_icon_target5; ?>" href="<?php echo esc_url( $custom_url ); ?>"><i class="fa <?php echo $custom_icon; ?>"></i></a> </li>
                <?php
            endif;
            ?>
            </ul>
...
function rab_sc_testimonial ($atts, $content = null) {
    ...
    <div id="<?php echo esc_attr($id); ?>" class="testimonials <?php echo esc_attr($testimonialWithAnimation); if ($style == 'light') { echo ' skin-light'; }?>" data-id="<?php echo esc_attr($id);?>" <?php echo esc_attr($animation_params); ?>>
        <div class="quot-icofont icofont-container">
            <span class="quot-icon"></span>
            <span class="quot-icon"></span>
        </div>
        <div class="swiper-container swiper-container-<?php echo esc_attr($id)?> clearfix">
...
function rab_sc_testimonial_item ($atts, $content = null) {
...
function rab_sc_progressbar($atts ,$content=null)
{
    extract(shortcode_atts(array(
        'title'    => '',
        'title_color'    => '',
...
function rab_sc_social_icon ($atts, $content=null)
{
    extract(shortcode_atts(array(
        'sociallink_url' => '',
	...
	if(esc_attr($sociallink_type)=='custom') {
        ...
        <style type="text/css" media="all">
            .<?php echo esc_attr($sociallink_type); ?> a:before {
		        ...
		        span.icon.icofont .icofont-<?php echo esc_attr($sociallink_type); ?>{
    ...
    <span class="rab-social-link <?php echo esc_attr($sociallink_type); ?>">
        <a id="<?php echo esc_attr($id); ?>" href="<?php echo esc_url($sociallink_url); ?>" target="_blank">
            <span class="icon icofont icofont-<?php echo esc_attr($sociallink_type); ?>"></span>
        </a>
    </span>
...
function rab_sc_custom_textbox($atts)
{
    extract(shortcode_atts(array(
...
function rab_sc_customTitle($atts)
{
    extract(shortcode_atts(array(
        'title'  => '',
        'title_color'  => '',
    ...
    <div id="<?php echo esc_attr($id); ?>" class="custom-title <?php echo implode(' ', $classes); ?>" >
    
        <?php if(strlen($title)) { ?>
            <div class="title-main text-uppercase text-center" style="<?php if(strlen($title_color)) { ?>color: <?php echo esc_attr($title_color); ?>; <?php } ?>">
                <h2><?php echo wp_kses( $title, $GLOBALS["allowed_tags"] ); ?></h2>
            </div>
...
function rab_sc_animated_text($atts){
    extract(shortcode_atts(array(
        'image_url'  => '',
        'title'  => '',
...
function rab_sc_custom_imagebox($atts)
{
    extract(shortcode_atts(array(
...
/*-----------------------------------------------------------------------------------*/
/*  icofont icofont-Box-top No border 
/*-----------------------------------------------------------------------------------*/

function rab_sc_iconbox_noborder($atts, $content=null) {
    ...
    $id     = rab_sc_id('iconbox_top');
    $class  = array('rab-iconbox',  'box', 'text-center');


    $inline_style = '';
    if( strlen( $icon_color ) ) {
        $inline_style .= ' style="color:'.esc_attr($icon_color).'"';
    }
    ob_start();
    ?>
    <div id="<?php echo esc_attr($id); ?>" class="<?php echo implode(' ', $class); ?>">                        
        <h4 class="contact-title">
            ...
            if( $icon_type === 'custom' ):
                ...
                $icon_class = 'icofont icofont-'.esc_attr($icon);
            endif;
            ?>
            <span class="<?php echo $icon_class; ?>" <?php echo $inline_style; ?>></span>
            <?php echo $title; ?>
        </h4>
...
/*-----------------------------------------------------------------------------------*/
/*  icofont icofont-Box-left
/*-----------------------------------------------------------------------------------*/

function rab_sc_iconbox_left($atts, $content=null)
{
    extract(shortcode_atts(array(
        'title'         => '',
        'title_color'   => '',
    ...
    <div id="<?php echo esc_attr($id); ?>" class="<?php echo implode(' ', $class); ?>"  <?php if(strlen(esc_attr($icon_animation))) { ?> data-delay="<?php echo esc_attr($icon_animation_delay); ?>" data-animation="<?php echo esc_attr($icon_animation); ?>" <?php } ?>>
        <div class="wrap">
            <div class="icon">
                <i class="icofont icofont-<?php echo esc_attr($icon); ?>"></i>
            </div>
            
            <!--figure-->
            <div class="content-wrap">
                <?php if(esc_attr($hasTitle)){ ?>
                    <h6 class="title font-ar"><?php echo esc_attr($title); ?></h6>
                <?php } ?>
                <!-- iconbox content -->
...
function rab_sc_conterbox($atts, $content=null)
{
    extract(shortcode_atts(array(
        'counter_number'  => '500',
        ...
        'counter_text'   =>  esc_html__('Description', 'rab'),
        'counter_text2'   =>  '' ,
...
function rab_sc_embed_video($atts)
{
    extract(shortcode_atts(array(
    ...
    <?php if( $video_display_type == 'local_video_popup' || $video_display_type == 'embeded_video_youtube_popup' || $video_display_type == 'embeded_video_vimeo_popup' ) { ?>
        ...
        <div id="<?php echo esc_attr($id); ?>" class="video_embed_container <?php if( $animation != 'none') { ?>  shortcodeAnimation <?php } ?>"  <?php if(strlen(esc_attr($animation))) { ?> data-delay="<?php echo esc_attr($delay); ?>" data-animation="<?php echo esc_attr($animation); ?>" <?php } ?>>
            ...
            <?php if( $video_display_type == 'local_video_popup' ) { ?>
                ...
                <a data-html="#video<?php echo esc_attr($id); ?>">
                    ...
                    <div class="play-button <?php echo esc_attr($video_play_button_color); ?>">
                        <span class="icon  icofont icofont-play2"></span>
                    </div>
                    <?php
                    if($text != '')
...
function rab_sc_audio_soundcloud($atts, $content=null)
{
    extract(shortcode_atts(array(
        'soundcloud_id'  => '',
...
function rab_sc_tab_group($atts, $content=null)
{
    $GLOBALS['rab_sc_tab'] = array();
    do_shortcode($content);
    $tabs = $GLOBALS['rab_sc_tab'];
...
function rab_sc_tab($atts, $content=null)
{
    $tabCnt = count($GLOBALS['rab_sc_tab']) + 1;

    extract(shortcode_atts(array(
        'title' => "Tab $tabCnt",
    ), $atts));

    $GLOBALS['rab_sc_tab'][] = array($title, do_shortcode($content));
}
...
function rab_sc_button($atts, $content = null)
{
    extract(shortcode_atts(array(
        'title'            => '',
        'text'             => esc_html__('View Page', 'rab'),
        'url'              => '#',
        'size'           => 'small',
        'button_text_color'  => '',
    ...
    <?php } ?>
        ...
        <a id="<?php echo esc_attr($id); ?>" class="<?php echo implode(' ', $class); ?>"  <?php if(strlen($button_text_color)) { ?> style="color: <?php echo esc_attr($button_text_color); ?>;" <?php } ?> href="<?php echo esc_url($link['url']); ?>" title="<?php echo esc_attr($title); ?>" <?php if($link['target'] != '') {?>target="<?php echo esc_attr( $link['target'] ); ?>"<?php } ?>>
            ...
            <?php if(strlen($button_icon)) { ?>
                <span class="icon" <?php if(strlen($button_icon_position)) { ?> data-float="<?php echo esc_attr($button_icon_position); ?>" <?php } ?> data-hover="icofont icofont-<?php echo esc_attr($button_icon); ?>">
                    <span class="icofont icofont-<?php echo esc_attr($button_icon); ?>"></span>
                </span>
            <?php } ?>
...
function rab_sc_vctoggle($atts, $content=null)
{
    extract(shortcode_atts(array(
        'title' => '',
        'open'  => 'false',
    ...
    <div class="toggle_wrap<?php if ($open == 'true') { ?> wpb_toggle_open  <?php } ?>">
        <div class="wpb_toggle<?php if ($open == 'true') { ?> wpb_toggle_title_active<?php } ?>">
            <div class="border-bottom">
                <div class="icon icofont icofont-plus"></div>
                <div class="icon icofont icofont-minus"></div>
                <div class="title"><?php echo esc_attr($title); ?></div>
            </div>
        </div>
...
function rab_sc_imagecarousel($atts, $content=null)
{
    extract(shortcode_atts(array(
        "nav_style" => "dark",
        "images"   => "",
    ...
    <div id="<?php echo esc_attr($id); ?>" class="carousel <?php if ( $enterance_animation != 'default') { ?> hasAnimation <?php } ?> <?php echo esc_attr($enterance_animation); ?> <?php echo esc_attr($gutter); ?> <?php if (strlen($nav_style)) { echo esc_attr($nav_style); }?> <?php echo ($zoom != ""? "zoom-hover":""); ?>" data-id="<?php echo esc_attr($id); ?>">
            ...
            <div class="swiper-wrapper">
                ...
                ?>
                    ...
                    <div class="swiper-slide carousel_item">
                        <div class="image-container lazy-load lazy-load-on-load" style="padding-top:<?php echo rab_get_height_percentage($image); ?>%">
                            <?php
                            //sanitization performed in above lines!
                            echo $image; ?>
                        </div>
...
function rab_sc_showcase($atts,$content)
{
    extract(shortcode_atts(array(
...
<div id="<?php echo esc_attr($id); ?>" class="showcase ep_lightGallery <?php echo esc_attr($direction) . ' '. esc_attr($style) . ' ' . esc_attr($bg_effect); ?> " data-effect="<?php echo esc_attr($bg_effect) ?>">
    ...
    <div class="showcase-content-container">
        ...
        <div class="container container-next-showcase">
            <span class="next-showcase">
                <a href="#" title="<?php esc_attr_e("NEXT",'rab') ?>"></a>
            </span>
        </div>
        <?php
        }
...
function rab_sc_showcase_item($atts)
{
    extract(shortcode_atts(array(
            ...
            ?>
                    <div class="item-pics  <?php if( esc_attr($text_bg) == "show" ) { echo "had_text_bg"; } ?>">
                        <div class="swiper-container">
                            <div class="swiper-wrapper rab_lightGallery">
                            <?php
                                foreach($image_ids as $image_id) {
...
function rab_sc_gallery($atts)
{
    ...
    if(count($catArr) == 0 || count($catArr) > 1)
    {
        if ( $portfolio_filter == 'all' ) {
            ...
            $listCatsArgs = array('title_li' => '', 'taxonomy' => 'gallery_cat', 'walker' => new rab_portfolio_walker(), 'echo' => 0, 'include' => '' );
        } else {
             // Generate  Selected Taxonomy
            $listCatsArgs = array('title_li' => '', 'taxonomy' => 'gallery_cat', 'walker' => new rab_portfolio_walker(), 'echo' => 0, 'include' => implode("," , $filters ));           
        }


        $catList = '<li class="current"><span class="filter_item active" data-filter="*" >'.esc_html__('All', 'rab').'<span class="filterline"></span><span class="post-count">'. sprintf("%02d", $portfolioItemNumber) .'</span></span></li>';
    ...
    <div id="<?php echo esc_attr($portfolioId); ?>" data-portfolio-type="<?php echo esc_attr($type); ?>" data-value="<?php echo esc_attr($portfolioId); ?>"  data-id="<?php echo esc_attr($id); ?>" data-startPage="<?php echo esc_attr($ppaged); ?>" data-maxPages="<?php echo esc_attr($maxPages); ?>" data-nextLink="<?php echo next_posts($pMax, false); ?>" class="epicoSection portfolioSection <?php echo esc_attr($filter_loadmore_style); ?> <?php echo esc_attr($type); ?> <?php if ($portfolio_hover_like_button == 'hide') { ?> hideLikeBtn <?php } ?>" data-layout-style="<?php echo esc_attr($portfolio_masonry);?>">
        ...
        <div  class="portfoliowrap wrap">
            ...
            <?php  if ( $filter_display == 'show' ){
                    ...
                    <div class="hidden-desktop clearfix">
                        <ul class="filterstablet responsive_filter_dropdown portfolio-filter" data-option-key="filter">
                            <li class="">
                                <div>
                                    <span class="text"><?php esc_html_e('All', 'rab') ?></span>
                                    <span class="icon icofont icofont-angle-down"></span>
                                </div>
            ...
            <div class="portfolio_wrap">
                ...
                <!-- gallery items  -->
                <div  class="isotope <?php echo esc_attr($portfolio_hover) . ' ' . esc_attr($portfolio_hover_style). ' ' . esc_attr($enterance_animation).' '. esc_attr($gallery_pop_up);?>" data-categories="<?php if($portfolio_filter == 'all' ) { echo 'all'; } else {echo implode(" ",$filters); } ?>" data-animation-type="<?php echo esc_attr($animation_style);?>">
                    <div id="<?php echo esc_attr($portfolioLoop); ?>" class="ep_lightGallery">
                        ...
                        <?php
                            ...
                            while ($query->have_posts()) {
								...
								rab_get_template_part( 'templates/gallery-thumbnail', [ '$portolio_type' => $portolio_type  , '$portfolioLoop' => $portfolioLoop , '$count' => $count , '$portfolio_masonry' => $portfolio_masonry  ] );
                    ...
                    <div class="pLoadMore <?php echo esc_attr($pLoadMore); ?>  clearfix">
                        <div class="readmore clearfix">
                            <div class="loadMore" data-id="<?php echo esc_attr($id); ?>">
                                <span class="text load-more-text"><?php esc_html_e("Load more",'rab'); ?></span>
                                <span class="text loading-text"><?php esc_html_e("Loading...",'rab'); ?></span>
                            </div>
                        </div>
                    </div>
...
function rab_sc_newsletter($atts)
{
...
/*-----------------------------------------------------------------------------------*/
    ...
    function rab_sc_product( $atts ) {
        ...
        return rab_product_loop( $query_args, $atts, 'single_product' );
    }


    //Single product 2
    function rab_sc_product_2( $atts ) {
        global $woocommerce;
        extract(shortcode_atts(array(
                    ...
                    <li  <?php post_class( $class); echo $style; ?>>
                        ...
                        if($badges != 'disable')
                        {
                            if ( $product -> is_on_sale() ) {
                                ...
                                echo apply_filters( 'woocommerce_sale_flash', '<span class="onsale">' . esc_html__( 'Sale!', 'rab' ) . '</span>', $post, $product );
                            
                            } 

                            if ( ! $product-> is_in_stock() ) {          
                                echo '<div class="out_of_stock_badge_loop">' . esc_html__( 'Out of stock', 'rab' ) .'</div>';            
                            }
                            ...
                            <div class="add_to_cart_btn_wrap   <?php if($image == 'disable') {  ?> disableImageModernSingleProduct <?php } ?>">
                                ...
                                <?php if($image != 'disable') {  ?>
                                    ...
                                    <div class="images">
                                        ...
                                        ?>
                                        <div class="product-fullview-thumbs <?php echo( $image_num == 1 ? 'lazy-load lazy-load-on-load" style="padding-top:' . rab_get_height_percentage($image) .'%;"':'') ?>">
                                            <?php
                                            if( $image_num == 1 ) {
                                                echo $image;//img tag
                                            }
                                            ...
                                            <div class="swiper-container clearfix">
                                                <div class="swiper-wrapper">
                                                    ...
                                                    foreach ( $attachment_ids as $attachment_id ) {
                                                        ...
                                                        //echo slide
                                                        $slide = '<div class="swiper-slide lazy-load lazy-load-on-load" style="padding-top:' . rab_get_height_percentage($image) .'%;">' . $image .'</div>';
                                ...
                                <div class="product-buttons">
                                    ...
                                    echo apply_filters( 'woocommerce_loop_add_to_cart_link',
                                        sprintf( '<span class="product-button product_type_%s"><a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="button %s product_type_%s ajax_add_to_cart" title="%s">
                                                <span class="icon"></span>
                                                <span class="firts_text txt hidden-v-tablet hidden-phone">%s</span>
                                                <span class="secound_txt txt hidden-v-tablet hidden-phone">%s</span>
                                            </a></span>',
    ...
    function rab_products( $atts ) {
        ...
        return rab_product_loop( $query_args, $atts, 'products' );
    }

    function rab_recent_products( $atts ) {
        $atts = shortcode_atts( array(
            'per_page'  => '12',
            'columns'   => '1',
            'orderby'   => 'date',
        ...
        return rab_product_loop( $query_args, $atts, 'recent_products' );
    }

    function rab_sale_products( $atts ) {
        $atts = shortcode_atts( array(
            'per_page' => '12',
            'columns'  => '1',
            'orderby'  => 'title',
        ...
        return rab_product_loop( $query_args, $atts, 'sale_products' );
    }

    function rab_best_selling_products( $atts ) {
        $atts = shortcode_atts( array(
            'per_page' => '12',
            'columns'  => '1',
            'enterance_animation' => 'fadeIn',
        ...
        return rab_product_loop( $query_args, $atts, 'best_selling_products' );
    }

    function rab_top_rated_products( $atts ) {
        $atts = shortcode_atts( array(
            'per_page' => '12',
            'columns'  => '1',
            'orderby'  => 'title',
        ...
        $return = rab_product_loop( $query_args, $atts, 'top_rated_products' );
    ...
    function rab_featured_products( $atts ) {
        $atts = shortcode_atts( array(
            'per_page' => '12',
            'columns'  => '1',
            'orderby'  => 'date',
        ...
        return rab_product_loop( $query_args, $atts, 'featured_products' );
    }

    function rab_product_attribute( $atts ) {
        $atts = shortcode_atts( array(
            'per_page'  => '12',
            'columns'   => '4',
            'orderby'   => 'date',
        ...
        return rab_product_loop( $query_args, $atts, 'product_attribute' );
    }

    function rab_product_category( $atts ) {
        $atts = shortcode_atts( array(
            'per_page' => '12',
            'columns'  => '1',
            'orderby'  => 'title',
        ...
        $return =  rab_product_loop( $query_args, $atts, 'product_cat' );
    ...
    function rab_product_loop( $query_args, $atts, $loop_name ) {
                ...
                <li <?php post_class( $class); ?>>
                    <div class="productwrap">
                        ...
                        <div class="add_to_cart_btn_wrap lazy-load-hover-container">
                            <?php echo '<a href="' . esc_url(get_the_permalink()) . '" class="product-link" title="' . esc_attr(get_the_title()) .'"></a>'; 
                            ...
                            ?>
                            <div class="imageswrap productthumbnail lazy-load lazy-load-on-load" style="padding-top: <?php echo esc_attr(rab_get_height_percentage($image)); ?>%;">
                                <?php
                                //Sanitization performed in above lines!
                                echo $image;
                                ?>
            ...
            <?php
                ...
                <li <?php post_class( $class); ?>>
                    <div class="productwrap">
                        ...
                        <div class="add_to_cart_btn_wrap lazy-load-hover-container">
                            <?php echo '<a href="' . esc_url(get_the_permalink()) . '" class="product-link" title="' . esc_attr(get_the_title()) .'"><span class="hidden-desktop">' . esc_html__('go to detail','rab') .'</span></a>'; 
                            ...
                            ?>
                            <div class="imageswrap productthumbnail lazy-load lazy-load-on-load" style="padding-top: <?php echo rab_get_height_percentage($image); ?>%;">
                                <?php
                    ...
                    <div class="productwrap">
                        <div class="add_to_cart_btn_wrap lazy-load-hover-container">
                            ...
                            <div class="hover-content no-select">
                                ...
                                echo '<a href="' . esc_url(get_the_permalink()) . '" class="product-link" title="' . esc_attr__('Buy now','rab') .'">' . esc_html__('Buy now','rab') .'</a>';
                ...
                <li <?php post_class( $class); ?>>
                    <div class="productwrap">
                        ...
                        <div class="wrap_after_thumbnail">
                            <?php
                            echo '<h3 class="instant_shop_heading">' . get_the_title() . '</h3>';
                            ?>
                            ...
                            <div class="instant_shop_button">
                                ...
                                <?php if ( $price_html = $product->get_price_html() ) { ?>
                                    ...
                                    <?php
                                        echo apply_filters( 'woocommerce_loop_add_to_cart_link',
                                            sprintf( '<span class="product_type_%s"><a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="%s product_type_%s ajax_add_to_cart" title="%s">
                                                    <span class="txt">%s</span>
                                                </a></span>',
                                                esc_attr( $product->get_type() ),
                                                esc_url( $product->add_to_cart_url() ),
    ...
    //Product categories
    function rab_product_categories ( $atts ) {
        global $woocommerce_loop;
    ...
    function rab_product_page( $atts ) {
        if ( empty( $atts ) ) {
            return '';
        }
        ...
        while ( $single_product->have_posts() ) : $single_product->the_post(); ?>
            ...
            <div class="infoOnHover" data-product-page-preselected-id="<?php echo esc_attr( $preselected_id ); ?>">

                <?php wc_get_template( 'content-single-product.php',array('is_product_shortcode' => true ) ); ?>
        ...
        if(function_exists('is_woocommerce')) {
            ...
            $classes[] = 'shop-'. cs_get_option('shop-column') .'column';
...
function rab_sc_instgram_feed($atts)
{
    extract(shortcode_atts(array(
        'user' => 'self',
        'otheruser'=>'',
    ...
    $id = rab_sc_id('instagram_feed');


    // get ep-core plugin directory address and check is active or not ?
    $files = scandir(WP_PLUGIN_DIR);
    $ep_core = false;
    $ep_coreDirectory = "";

    foreach ($files as $file ) {
        if($file != '.' && $file != '..' && $ep_core != true && is_dir(WP_PLUGIN_DIR.'/'.$file)){
            $filename = WP_PLUGIN_DIR.'/'.$file.'/ep-core.php';
            if (file_exists($filename)) {
                $ep_core = true;
                $ep_coreDirectory  =  $file;
            }
        }
    }

    if (!is_plugin_active($ep_coreDirectory.'/ep-core.php') || !class_exists('MetzWeb\Instagram\\Instagram'))
        return;
    ...
    try{
        ...
        if(cs_get_option('instagram_access_token') == '')
        {
            echo esc_html__("Please get your instagram access token in theme settings","rab");
            return;
        }

        $instagram->setAccessToken(cs_get_option('instagram_access_token'));
        ...
        <div class="instagram-feed <?php echo  $enterance_animation ; ?>" id="<?php echo esc_attr($id); ?>">
            <?php if($profile_info == 'enable')
            {
                ...
                <div class="header">
                    ...
                    echo '<a class="fallowme" href="http://instagram.com/' .  esc_attr($username) .'">' . esc_html__('Follow on instagram', 'rab') . '</a>';
                    ...
                    echo '</span>';
                    echo '<div class="info">';
                    echo '<span><span class="value">' . esc_html($media) . '</span>' . esc_html__('Posts', 'rab') .'</span>';
                    echo '<span><span class="value">' . esc_html($follower) . '</span>' . esc_html__('Followers', 'rab') .'</span>';
                    echo '<span><span class="value">' . esc_html($followed) . '</span>' . esc_html__('Following', 'rab') .'</span>';
                    echo '</div>';
                    echo '</div>';
    ...
    {
        echo '<p>' . esc_html__("An error occured, Maybe missed configuration!","rab") . '</p>';
    }
    catch(CurlException $e)
    {
        echo '<p>' . esc_html__("An error occured, Curl exception has been thrown!","rab") . '</p>';
    }
    catch(Exception $e)
    {
        echo '<p>' . esc_html__("An error occured, exception has been thrown!","rab") . '</p>';
    }
}
...
function rab_sc_blog_masonry( $atts, $content = null ){
    $query=$output=$width=$subStr=$style = $col
    =$blog_accent_color=$blog_post_number=$blog_text_accent_color=
    $blog_category=$blog_foreground_color=$blog_background_color=$id=$blog_column=$blog_bg='' ;
    $list=$day=array();
    $i=0;
    ...
    <div id="<?php echo esc_attr($id) ?>" class="blogLoop <?php echo esc_attr($id);?> masonry-blog isotope clearfix <?php echo esc_attr($blog_style);?> blogcolumn<?php echo esc_attr($blog_column); ?>   <?php if ( $enterance_animation != 'default') { ?> hasAnimation <?php } ?> <?php echo esc_attr($enterance_animation); ?>" data-columnnumber="<?php echo esc_attr($blog_column); ?>" data-layoutmode="<?php echo esc_attr($blog_layout_mode); ?>" data-id="<?php echo esc_attr($id) ?>" data-page="1" data-maxpages="<?php echo esc_attr($max)?>">
        ...
        <?php while ($query->have_posts()) {
            ...
            $thumb = wp_get_attachment_image_src( get_post_thumbnail_id() , 'rab_blog_thumbnail-fixed-height' )[0];
        ...
        <div class="isotope-item">
                                           ...
                                           <div class="play-button-wrap">
                                                <a>
                                                    <div class="play-button <?php echo esc_attr($blog_multimedia_icon_style); ?>">
                                                        <span class="icon icofont icofont-music-note3"></span>
                                                    </div>
                                                </a>
                                            </div>
                                            <?php
                            ...
                            <div class=" swiper-container clearfix" data-visibleitems="1">            
                                <div class="slides swiper-wrapper">
                                    ...
                                    foreach($images as $img){
                                     ?>
                                        <div class="swiper-slide">
                                            <div class="blogGalleryContainer" style="background-image:url(<?php print_r(esc_url($img)); ?>);">
                                ...
                                <div class="video-format"  data-id="<?php echo esc_attr($id); ?>"> 
                                    <div class="video_buttons_wrap">
									    <a class="galleryItem" href="<?php echo esc_attr($videoUrl); ?>">
											<div class="image-container lazy-load lazy-load-on-load" style="padding-top:<?php echo esc_attr(rab_get_height_percentage($image)); ?>%">
												<?php
                                                //Sanitization performed in above lines!
                                                echo $image; ?>
											</div>
                 ...
                 if($format=='quote') {
                    ...
                    <div class="blog-masonry-content">
                        ...
                        <a class="link_to_details" href="<?php the_permalink(); ?>">
                            ...
                            <p class="blog-excerpt"> <?php echo substr(get_the_excerpt(),0, 100).$subStr; ?></p>
                            </a>
                        <?php 
                        }
                        else
                        {
                            ...
                            if (!empty( $quote_content )&& !empty( $quote_author ) ) {
                                echo '<div class="icon ep-icon icofont icofont-quotes-right"></div>';
                            }?>
                     ...
                     if( $format!='quote') { ?>
                        <?php if($blog_category_author=='yes') { ?>
                            <div class="post-author-meta">
                                ...
                                <span class="ep-icon icofont icofont-user"></span>
                                <span class="post-author "><?php the_author_posts_link(); ?></span>
                                <span class="ep-icon icofont icofont-bubble"></span>
                                <span class="meta-comment-count"><a href="<?php comments_link(); ?>"> <?php comments_number( esc_html__('0', 'rab'), esc_html__('1', 'rab'), esc_html__('%', 'rab') ); ?></a></span>
            ...
            <?php } ?>
                ...
                <div class="pageNavigation cartBlog clearfix <?php echo esc_attr($load_more_style); ?>">
                    <div class="navNext"><?php next_posts_link(__('&larr; Older Entries', 'rab')) ?></div>
                    <div class="navPrevious"><?php previous_posts_link(__('Newer Entries &rarr;', 'rab')) ?></div>
                </div>
